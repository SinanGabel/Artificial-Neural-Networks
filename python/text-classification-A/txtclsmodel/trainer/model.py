...
# In tensorflow 1.12 the tensorflow reference may be different, this has been run on 1.8

# The (keras) python code is not usable in google cloud ml engine, they must be re-coded to tensorflow code

# ... except the keras estimator is actually re-coded to tensorflow code below

# For tensorflow code version refer to: model_native.py model code
...
def keras_estimator(model_dir,
    ...
    estimator = tf.keras.estimator.model_to_estimator(keras_model = model, model_dir=model_dir, config=config) # TODO: convert keras model to tf.estimator.Estimator
...
def train_and_evaluate(output_dir, hparams):
    ...
    estimator = keras_estimator(output_dir,
                    run_config,
                    learning_rate=hparams['learning_rate'],
                    embedding_path=hparams['embedding_path'],
                    word_index=tokenizer.word_index)  # TODO: create estimator
